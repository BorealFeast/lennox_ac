...
use esp_idf_hal::{
    gpio,
    rmt::{config::{TransmitConfig, Loop}, FixedLengthSignal, PinState, Pulse, TxRmtDriver, CHANNEL0},
};
...
impl WS2812RMT<'_> {
    ...
    pub fn new(led: gpio::Gpio2, channel: CHANNEL0) -> Result<Self> {
        let config = TransmitConfig::new().clock_divider(16).looping(Loop::Endless);
